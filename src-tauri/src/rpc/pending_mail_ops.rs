use crate::rpc::gmail_labels::gmail_flag_label_delta;
use crate::state::AppState;
use pebble_core::traits::{FolderProvider, MailTransport};
...
use super::messages::{
    ...
    modify_gmail_labels_with_auth_retry, refresh_search_document, remove_search_documents,
};
...
pub async fn run_pending_mail_ops_worker(state: std::sync::Arc<AppState>) {
    let mut interval =
    ...
    loop {
        ...
        if let Err(e) = process_pending_mail_ops(&state).await {
            warn!("Pending mail op worker pass failed: {e}");
        }
    }
}

pub async fn process_pending_mail_ops(state: &AppState) -> std::result::Result<usize, PebbleError> {
    let ops = state
        .store
...
fn emit_pending_ops_changed(state: &crate::state::AppState) {
    state.emit("mail:pending-ops-changed", serde_json::json!({}));
}
...
async fn replay_remote_update_flags(
    ...
    match provider_type {
        ProviderType::Gmail => {
            let add = string_array(payload, "add_labels")
                .unwrap_or_else(|| gmail_flag_label_delta(is_read, is_starred).add_labels);
            let remove = string_array(payload, "remove_labels")
                .unwrap_or_else(|| gmail_flag_label_delta(is_read, is_starred).remove_labels);
            if add.is_empty() && remove.is_empty() {
                return Ok(());
            }
            let provider = connect_gmail(state, &message.account_id).await?;
            modify_gmail_labels_with_auth_retry(
                state,
                &message.account_id,
                &provider,
                &message.remote_id,
