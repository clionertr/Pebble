...
pub(crate) fn gmail_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        redirect_url: oauth_config_value(
            "OAUTH_REDIRECT_URL",
            option_env!("OAUTH_REDIRECT_URL"),
            "http://127.0.0.1:1420/auth/callback",
        ),
        extra_auth_params: std::collections::HashMap::from([
...
pub(crate) fn provider_type(provider: &str) -> Result<ProviderType, PebbleError> {
...
pub(crate) async fn refresh_account_oauth_auth(
    state: &AppState,
    account_id: &str,
    provider: &str,
    ...
    let refresh_token = stored.refresh_token.clone().ok_or_else(|| {
        PebbleError::TokenRefreshFailed(format!(
            "No refresh token available for {provider} account {account_id}"
        ))
    })?;
...
pub(crate) async fn ensure_account_oauth_auth(
    ...
    if needs_refresh {
        return refresh_account_oauth_auth(state, account_id, provider).await;
    }
...
pub async fn complete_oauth_flow(
    ...
    Err(PebbleError::Internal(
        "OAuth is now handled via /auth/login endpoints".to_string(),
    ))
}
