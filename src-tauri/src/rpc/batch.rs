...
use super::messages::{
    find_folder_by_role, find_message_folder, modify_gmail_labels_with_auth_retry,
                ...
                Ok(conn) => {
                    match &conn {
                        ConnectedProvider::Gmail(provider) => {
                            for msg in messages {
                                match modify_gmail_labels_with_auth_retry(
                                    &state,
                                    account_id,
                                    provider,
                                    &msg.remote_id,
    ...
    for (account_id, (provider_type, messages)) in &groups {
        match ConnectedProvider::connect(&state, account_id, provider_type).await {
            Ok(conn) => {
                match &conn {
                    ConnectedProvider::Gmail(provider) => {
                        let delta = gmail_flag_label_delta(Some(is_read), None);
                        let add = delta.add_labels;
                        let remove = delta.remove_labels;
                        for msg in messages {
                            match modify_gmail_labels_with_auth_retry(
                                &state,
                                account_id,
    ...
    for (account_id, (provider_type, messages)) in &groups {
        match ConnectedProvider::connect(&state, account_id, provider_type).await {
            Ok(conn) => {
                match &conn {
                    ConnectedProvider::Gmail(provider) => {
                        let delta = gmail_flag_label_delta(None, Some(starred));
                        let add = delta.add_labels;
                        let remove = delta.remove_labels;
                        for msg in messages {
                            match modify_gmail_labels_with_auth_retry(
