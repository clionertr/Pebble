...
- Border radius is hierarchical: `{rounded.md}` (16px) for buttons + inputs, `{rounded.lg}` (24px) for content + product cards, `{rounded.xl}` (32px) for the hero illustration container, `{rounded.pill}` for badges.
...
| Token | Value | Use |
|---|---|---|
| `{rounded.xs}` | 8px | Reserved for badge accents and tiny dropdowns |
| `{rounded.sm}` | 12px | Small inline buttons, dropdown items |
| `{rounded.md}` | 16px | Standard CTA buttons, text inputs, category tabs |
| `{rounded.lg}` | 24px | Content cards (feature, pricing, code-window, model-comparison) |
| `{rounded.xl}` | 32px | Hero illustration container, the larger marquee components |
...
**`button-primary`** — The signature coral CTA. Background `{colors.primary}` (#cc785c), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 20px, height 40px, rounded `{rounded.md}` (16px). Active state `button-primary-active` darkens to `{colors.primary-active}` (#a9583e).
...
**`text-input`** — Standard text input. Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-md}`, rounded `{rounded.md}` (16px), padding 10px × 14px, height 40px. 1px hairline border in `{colors.hairline}`.
