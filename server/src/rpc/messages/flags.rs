...
use super::{
    connect_gmail, connect_outlook, load_imap_config, modify_gmail_labels_with_auth_retry,
...
pub async fn update_message_flags(
    ...
    let outcome = match writeback_info {
        WritebackInfo::Gmail { msg } => {
            let delta = gmail_flag_label_delta(is_read, is_starred);
            ...
            if !add.is_empty() || !remove.is_empty() {
                ...
                match connect_gmail(&state, &msg.account_id).await {
                    Ok(provider) => match modify_gmail_labels_with_auth_retry(
                        &state,
                        &msg.account_id,
                        &provider,
                        &remote_id,
