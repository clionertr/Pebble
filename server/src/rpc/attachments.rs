...
pub async fn download_attachment(
    ...
    let actual_path = match run_blocking(move || {
        ...
        copy_attachment_file_safely(source_path, save_path, |bytes_copied, total_bytes| {
            let _ = tx.send(crate::state::EventPayload {
                event: "mail:attachment-download-progress".to_string(),
                payload: serde_json::json!({ "attachment_id": att_id, "bytes_copied": bytes_copied, "total_bytes": total_bytes }),
            });
        })
    })
    .await
