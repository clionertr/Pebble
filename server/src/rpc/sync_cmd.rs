...
use crate::rpc::sync_cmd_support::{
    all_account_ids, imap_capability_realtime_mode, imap_initial_realtime_mode,
    manual_realtime_status_payload, normalize_explicit_account_ids, polling_status_message,
    realtime_error_mode, realtime_preference_poll_interval, realtime_status_payload,
    should_dispatch_wake_trigger_after_ensure, should_drop_trigger_handle,
    should_send_stop_signal_to_handle, should_start_missing_worker_for_trigger_route,
    should_start_missing_worker_for_wake, RealtimePreferenceStartSummary, SyncStartOutcome,
...
pub use crate::rpc::sync_cmd_support::{SyncWakeFailure, SyncWakeRequest, SyncWakeResult};
...
pub async fn resume_all_syncs(state: axum::extract::State<std::sync::Arc<crate::state::AppState>>) {
    ...
    for account in accounts {
        ...
        if let Err(e) = start_sync_inner(state.0.clone(), account.id.clone(), None).await {
...
fn emit_realtime_status(state: &crate::state::AppState, payload: RealtimeStatusPayload) {
    state.emit("mail:realtime-status", payload);
}
...
fn build_sync_task(
    ...
    let task = match account.provider {
        ...
        ProviderType::Imap => {
            // --- IMAP path ---
            let imap_config = match crate::rpc::messages::load_imap_config(
                &state.store,
                &state.crypto,
                &account_id_clone,
            ) {
...
pub async fn stop_sync(
    ...
    stop_sync_inner(state.0.clone(), &account_id).await;
    Ok(())
}

async fn stop_sync_inner(state: std::sync::Arc<AppState>, account_id: &str) {
    let mut handles = state.sync_handles.lock().await;
    if let Some(handle) = handles.remove(account_id) {
