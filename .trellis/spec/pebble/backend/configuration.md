...
- **源码直接运行**：后端启动时会读取当前工作目录的 `.env`，但已经存在的进程环境变量优先；开发调试应从仓库根目录启动，确保 `./data`、`./dist` 和 `.env` 路径一致。
- **Docker 部署**：Compose 必须设置 `PEBBLE_HOST=0.0.0.0`，并默认把容器内 `3000` 映射到宿主机 `127.0.0.1:9191`。
- **公网部署**：推荐只暴露宿主机反代入口；外部 nginx/Caddy/1Panel OpenResty/Cloudflare Tunnel 统一转发到 `http://127.0.0.1:9191`。
- **浏览器通知**：Web Push 生产环境需要 HTTPS 或浏览器认可的安全上下文；localhost 仅用于开发例外。
- **单进程数据目录**：同一个 `data/` 目录只能有一个 Pebble 后端进程；`data/index/` 的 Tantivy writer lock 出现 `LockBusy` 时，先停止旧进程，不要并行运行 `cargo run` 和 Docker 容器。
...
### 1. Scope / Trigger
- Trigger: Rust 后端注入的 `Content-Security-Policy` 会直接影响 `ShadowDomEmail` 渲染出的邮件 HTML。
- 范围：`server/src/middleware/` 的安全响应头、`src/components/ShadowDomEmail.tsx` 的 Shadow DOM 样式加载，以及 `crates/pebble-privacy/src/sanitizer.rs` 的 CSS 白名单。

### 2. Signatures
- 应用响应头：
  ```http
  Content-Security-Policy: default-src 'self'; img-src 'self' data: https:; script-src 'self'; style-src 'self'; style-src-elem 'self'; style-src-attr 'unsafe-inline'; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'
  ```
...
- 禁止把 CSP 退回到全局 `style-src 'self' 'unsafe-inline'`。

### 4. Validation & Error Matrix
- CSP 缺少 `style-src-attr 'unsafe-inline'` -> Docker 生产环境会拦截邮件正文或 blocked image placeholder 的 `style=""`。
...
- 修改 CSP 后搜索 README、deploy 示例和最佳实践文档，确认没有残留旧策略。
...
- 范围：`.github/workflows/docker.yml`、`deploy/install.sh`、`deploy/compose.prod.yml`、`deploy/backend.Dockerfile`、`server/src/main.rs`、`server/src/static_assets.rs`。
...
- 生产 compose：`deploy/compose.prod.yml`，默认镜像 `ghcr.io/clionertr/pebble:latest`。
- 入口端口：单容器默认绑定 `127.0.0.1:9191:3000`。
...
- `.env` 必须写入 `PEBBLE_PASSWORD_HASH`、`OAUTH_REDIRECT_URL`、`PEBBLE_IMAGE`、`PEBBLE_HTTP_BIND`。
- Docker Compose 中 Pebble 服务必须设置 `PEBBLE_HOST=0.0.0.0`，容器内监听 `3000`，宿主机默认入口为 `127.0.0.1:9191`。
...
- 健康检查 `http://127.0.0.1:9191` 超时 -> 打印 `docker compose ps` 和 Pebble 服务最近日志。
...
### 5. Good/Base/Bad Cases
- Good: 用户运行安装脚本，输入 `https://mail.closev.com` 或接受自动探测的 IP URL；登录密码可以手动输入，也可以留空生成 32 位随机密码。脚本生成 `.env`、拉取 `latest` 镜像、启动单容器服务，并确认 `127.0.0.1:9191` 可访问。
- Base: 用户暂不配置 Google/Microsoft OAuth，`.env` 中 OAuth 字段为空，服务仍能启动；后续可编辑 `.env` 后重启。
- Bad: 将生产 compose 改回多容器前端服务或直接把容器 `3000` 暴露到公网，会破坏“一键部署单镜像”和默认只绑定回环地址的契约。

### 6. Tests Required
- `bash -n deploy/install.sh`。
- `docker compose -f deploy/compose.prod.yml config`，断言绑定 `127.0.0.1:9191:3000`。
