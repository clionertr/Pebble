...
在浏览器中打开 `http://localhost:1420`。Vite 开发服务器会自动将 `/rpc`、`/events` 和 `/auth` 的请求代理到 3000 端口的后端。
...
静态文件会输出到 `dist/` 目录。你可以使用任何 Web 服务器（nginx、caddy 等）来托管这些文件，并将 `/rpc`、`/events` 和 `/auth` 路径代理到 Rust 后端。
