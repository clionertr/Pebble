...
export default function ProxyTab() {
  ...
  useEffect(() => {
    let cancelled = false;
    const proxyAccountRefs = accountIdsKey
      ? accountIdsKey.split("|").map((entry) => {
          const [id, provider] = entry.split(":");
          return { id, provider: provider as Account["provider"] };
        })
      : [];
    const accountIds = new Set(proxyAccountRefs.map((account) => account.id));
    setAccountProxyDrafts((current) => {
      const next: Record<string, AccountProxyDraft> = {};
      for (const account of proxyAccountRefs) {
    ...
    proxyAccountRefs.forEach((account) => {
      const loadProxy =
    ...
    <div>
      ...
      >
        {t(
          ...
          "Used by translation, OAuth, Gmail/Outlook API requests, IMAP, and SMTP when an account does not define its own SOCKS5 proxy.",
        )}
      </p>
      <div
        role="group"
      ...
      >
        {t(
          "settings.accountProxiesDesc",
          "Override the global SOCKS5 proxy for individual accounts.",
        )}
      </p>

      {accounts.length === 0 ? (
        <p
          id="account-proxy-list"
          ...
          {accounts.map((account) => {
            ...
            return (
              ...
              >
                ...
                <div
                  role="group"
                  aria-label={t("settings.accountProxyMode", "Proxy mode")}
                ...
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      boxShadow:
                        draft.loading || draft.saving ? "none" : "0 1px 2px rgba(0, 0, 0, 0.12)",
                    }}
                  >
